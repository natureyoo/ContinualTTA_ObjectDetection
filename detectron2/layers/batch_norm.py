...
class FrozenBatchNorm2d(nn.Module):
    ...
    def __init__(self, num_features, eps=1e-5, out_batch_norm=False, out_batch_resolution=32):
        super().__init__()
        ...
        self.out_batch_resolution = out_batch_resolution
        self.mean = torch.Tensor([])
        self.var = torch.Tensor([])
        self.cnt = 0
        self.adapt_type = None
        self.min_momentum_constant = 0.0001
        self.decay_factor = 0.94
        self.mom_pre = 0.01
        self.source_sum = 128

    def forward(self, x):
        if self.adapt_type == "NORM":
            ...
            if self.adapt_type == 'DUA':
                self.running_mean = (1 - (self.mom_pre + self.min_momentum_constant)) * self.running_mean + (self.mom_pre + self.min_momentum_constant) * x.mean(dim=[0,2,3])
                self.running_var = (1 - (self.mom_pre + self.min_momentum_constant)) * self.running_var + (self.mom_pre + self.min_momentum_constant) * x.var(dim=[0,2,3], unbiased=True)
        ...
        if self.out_batch_norm:
            # _out = nn.AdaptiveMaxPool2d(self.out_batch_resolution)(out.clone())
            _out = out.clone()
            if not out.requires_grad:   # collect features
                if self.cnt == 0:
                    self.mean = _out.mean(dim=0).detach()
                    self.var = _out.var(dim=0).detach()
                else:
                    self.mean = self.mean * self.cnt + _out.mean(dim=0).detach() * out.shape[0]
                    self.var = self.var * self.cnt + _out.var(dim=0).detach() * out.shape[0]
                    self.cnt += out.shape[0]
                    self.mean /= self.cnt
                    self.var /= self.cnt
            else:   # adaptation
                self.mean = _out.mean(dim=0)
                self.var = _out.var(dim=0)
        return out
...
def get_norm(norm, out_channels, out_batch_norm=False, out_batch_resolution=32):
    """
    Args:
    ...
    if isinstance(norm, str):
        ...
        if isinstance(norm, BatchNorm2d):
            return norm(out_channels, out_batch_resolution=out_batch_resolution)

    # return norm(out_channels, out_batch_norm=out_batch_norm, out_batch_resolution=out_batch_resolution)
...
class LayerNorm(nn.LayerNorm):
    ...
    def __init__(self, normalized_shape, eps=1e-6, out_batch_norm=False, out_batch_resolution=32):
        super().__init__(normalized_shape, eps)
        self.out_batch_norm = out_batch_norm
        self.out_batch_resolution = out_batch_resolution
        self.max_pool = nn.AdaptiveMaxPool2d((self.out_batch_resolution, self.out_batch_resolution))
        self.mean = torch.Tensor([])
        self.var = torch.Tensor([])
        self.cnt = 0

    def update(self, x):
        x = self.max_pool(x.permute(0, 3, 2, 1))
        if not x.requires_grad:
            if self.cnt == 0:
                self.mean = x.mean(dim=0).detach()
                self.var = x.var(dim=0).detach()
            else:
                h, w = min(self.mean.shape[1], x.shape[1]), min(self.mean.shape[2], x.shape[2])
                self.mean = self.mean[:h, :w] * self.cnt + x.mean(dim=0)[:h, :w].detach() * x.shape[0]
                self.var = self.var[:h, :w] * self.cnt + x.var(dim=0)[:h, :w].detach() * x.shape[0]
            self.cnt += x.shape[0]
            self.mean /= self.cnt
            self.var /= self.cnt
        else:
