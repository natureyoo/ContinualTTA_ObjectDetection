...
def _create_text_labels(classes, scores, class_names, is_crowd=None):
    ...
    if classes is not None:
        if class_names is not None and len(class_names) > 0:
            labels = [class_names[i] if i < len(class_names) else "None" for i in classes]
        else:
    ...
    def draw_dataset_input(self, dic, dic_image_size, output_size):
        ...
        if annos:
            ...
            labels = _create_text_labels(
                ...
                scores=None,
                class_names=names,
                is_crowd=[0] * category_ids.shape[0],
            )
            self.overlay_instances(
                labels=labels, boxes=refined_boxes, masks=None, keypoints=None, assigned_colors=colors
            )
