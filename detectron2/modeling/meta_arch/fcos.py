...
class FCOS(DenseDetector):
    ...
    def __init__(
        ...
        *,
        ...
        self.gl_features = {}
        self.fg_features = {}
        self.s_stats = torch.load(source_feat_stats) if source_feat_stats is not None else {}
        self.template_cov, self.t_stats, self.ema_n = {'gl': {}, 'fg': {}}, {'gl': {}, 'fg': {}}, {}
        if self.s_stats is not None and self.gl_align is not None:
            self.template_cov['gl'], self.t_stats['gl'] = {}, {}
        ...
        if self.s_stats is not None and self.fg_align is not None:
            self.template_cov['fg'], self.t_stats['fg'] = {}, {}
    ...
    def forward(self, batched_inputs: List[Dict[str, Tensor]]):
        ...
        if self.training:
        ...
        elif self.online_adapt:
            adapt_loss = {}
            if self.gl_align is not None:
                loss_gl_align = 0
                if self.gl_align == "KL":
                    for idx, k in enumerate(self.head_in_features):
                        cur_feats = features[idx].mean(dim=[2, 3])
                        ...
                        t_dist = torch.distributions.MultivariateNormal(cur_target_mean,
                                                                        self.s_stats["gl"][k][1].to(self.device) +
                                                                        self.template_cov["gl"][k].to(self.device))
                        s_dist = torch.distributions.MultivariateNormal(self.s_stats["gl"][k][0].to(self.device),
                                                                        self.s_stats["gl"][k][1].to(self.device) +
                                                                        self.template_cov["gl"][k].to(self.device))
                        cur_loss_gl_align = (torch.distributions.kl.kl_divergence(s_dist,
                                                                                  t_dist) + torch.distributions.kl.kl_divergence(
                            t_dist, s_dist)) / 2
                ...
                #     cur_bn_stats = [(l.mean, l.var) for l in list(self.backbone.bottom_up.modules()) if
                #                     (isinstance(l, FrozenBatchNorm2d) or isinstance(l, LayerNorm)) and l.out_batch_norm]
                #     for idx in range(len(cur_bn_stats)):
                #         if cur_bn_stats[idx][0].shape[0] > 0 and cur_bn_stats[idx][0].shape[0] == \
                #                 self.s_stats["bn_stats"][idx][0].shape[0]:
                #             h, w = min(cur_bn_stats[idx][0].shape[1], self.s_stats["bn_stats"][idx][0].shape[1]), min(
                #                 cur_bn_stats[idx][0].shape[2], self.s_stats["bn_stats"][idx][0].shape[2])
                #             loss_gl_align += nn.L1Loss()(cur_bn_stats[idx][0][:, :h, :w],
                #                                          self.s_stats["bn_stats"][idx][0][:, :h, :w])
                #             loss_gl_align += self.alpha_fg * nn.L1Loss()(cur_bn_stats[idx][1][:, :h, :w],
                #                                                          self.s_stats["bn_stats"][idx][1][:, :h, :w])
                ...
                adapt_loss["global_align"] = loss_gl_align
            ...
            return processed_results, adapt_loss
        else:
            if self.collect_features:
                if len(self.gl_features) > 0 and self.gl_features['p3'].shape[0] == 2000:
                    feat_stats = {}
                    feat_stats["gl"] = {k: (self.gl_features[k].mean(dim=0), torch.cov(self.gl_features[k].T)) for k
                                        in self.gl_features}
                    # feat_stats["fg"] = {k: (model.fg_features[k].mean(dim=0), torch.cov(model.fg_features[k].T)) for k
                    #                     in model.fg_features}
                    # feat_stats["bn_stats"] = [(m.mean, m.var) for m in list(model.backbone.bottom_up.modules()) if (
                    #             isinstance(m, FrozenBatchNorm2d) or isinstance(m, LayerNorm)) and m.out_batch_norm]
                    feat_stats["kl_div"] = {}
                    for k in self.gl_features:
                        num = self.gl_features[k].shape[0]
                        dist1 = torch.distributions.MultivariateNormal(self.gl_features[k].mean(dim=0),
                                                                       torch.cov(self.gl_features[k].T))
                        dist2 = torch.distributions.MultivariateNormal(self.gl_features[k][:20, :].mean(dim=0),
                                                                       torch.cov(self.gl_features[k].T))
                        kl_div = (torch.distributions.kl.kl_divergence(dist1,
                                                                       dist2) + torch.distributions.kl.kl_divergence(
                            dist1, dist2)) / 2
                    ...
                    torch.save(feat_stats, os.path.join("models", "fcos_feature_stats.pt"))
                    exit()
                # collect backbone features
                elif (len(self.gl_features) > 0 and self.gl_features['p3'].shape[0] < 400) or len(self.gl_features) ==0:
                    for idx, k in enumerate(self.head_in_features):
                        cur_feats = features[idx].mean(dim=[2, 3]).detach()
